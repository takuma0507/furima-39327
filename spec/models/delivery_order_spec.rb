  ...
  describe '商品購入' do
    ...
    context '新規登録できないとき' do
        it '郵便番号が空では購入できない' do
          ...
          @deliveryorder.valid?
          expect(@deliveryorder.errors.full_messages).to include("郵便番号を入力してください")
        end
        
        it '郵便番号は、「3桁ハイフン4桁」の半角文字列でなければ登録できない' do
          @deliveryorder.post_code = '1234'
          @deliveryorder.valid?
          expect(@deliveryorder.errors.full_messages).to include("郵便番号は正確に入力してください")
        end
  
        it '都道府県が空では購入できない' do
          @deliveryorder.send_region_id = ' '
          @deliveryorder.valid?
          expect(@deliveryorder.errors.full_messages).to include("都道府県を入力してください")
        end

        it '都道府県に1が選択されている場合は出品できない' do
          @deliveryorder.send_region_id = '1'
          @deliveryorder.valid?
          expect(@deliveryorder.errors.full_messages).to include("都道府県を選択してください")
        end
        ...
        it '番地が空では購入できない' do
          ...
          @deliveryorder.valid?
          expect(@deliveryorder.errors.full_messages).to include("番地を入力してください")
        end

        it '電話番号が空では購入できない' do
            @deliveryorder.tel = ''
            @deliveryorder.valid?
            expect(@deliveryorder.errors.full_messages).to include("電話番号を入力してください")
        end

        it '電話番号が9桁以下では保存できない' do
          @deliveryorder.tel = '123456789' # 9桁の電話番号
          @deliveryorder.valid?
          expect(@deliveryorder.errors.full_messages).to include("電話番号は10桁から11桁の数字を入力してください")
        end
        ...
        it '電話番号は半角数字以外が含まれている場合は購入できない' do
          ...
          @deliveryorder.valid?
          expect(@deliveryorder.errors.full_messages).to include("電話番号は正確に入力してください")
        end

        it "tokenが空では購入できない" do
          @deliveryorder.token = nil
          @deliveryorder.valid?
          expect(@deliveryorder.errors.full_messages).to include("クレジットカード情報を入力してください")
        end
